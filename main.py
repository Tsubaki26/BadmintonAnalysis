...
class MatchAnalyzer:
    ...
    def get_perspective_trans_matrix(self, poslist1, poslist2):
        """
        射影変換行列を作成する。
        Argument:
        ・変換前の4点座標（左上から右回り）
    ...
    def analyze_match(
        self, match_video_path, is_save_video=False, output_path="./out.mp4"
    ):
        """
        試合動画の解析
        """
        ...
        while self.cap.isOpened():
            ...
            M = self.get_perspective_trans_matrix(poslist1, poslist2)
            ...
            """
            pos_xy_warp = []
            print(len(player_foot_pos_xy_list))
            ...
            if count % OUTPUT_INTERVAL == 0 and count >= TRACK_FRAMES:
                ...
                for pos in pos_xy_warp:
                    ...
                    self.court_img_for_output = cv2.add(
                        self.court_img_for_output, circle_add
                    )
